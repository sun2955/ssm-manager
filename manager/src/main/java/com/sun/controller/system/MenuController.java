...
public class MenuController  {
	...
	@ResponseBody
	public WebRespone getMenu(Long menuId) {
		...
		try {
			...
			e.printStackTrace();
			return WebRespone.error("获取信息失败，请联系网站管理员！");
		}
	}
	...
	@RequestMapping("menu/getUserMenu.do")
	@ResponseBody
	public WebRespone getUserMenu(String userName) {
		try {
			Tree<Menu> tree = menuService.getUserMenu(userName);
			WebRespone xx = WebRespone.success("获取菜单列表",tree);
			return xx;
		} catch (Exception e) {
			return WebRespone.error("获取用户菜单失败！");
		}
	}
